...
- The admin console is the same responsive client surface on iOS and Firebase Hosting `/admin`. `client/scripts/exportAdminWeb.js` creates ignored output under `hosting/admin`; build it immediately before Hosting deployment and never commit the generated bundle.
