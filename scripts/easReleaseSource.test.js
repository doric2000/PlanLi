...
test('a tracked Git archive excludes unrelated root config and detects source tampering', t => {
  ...
  const preparedArchives = fs.readdirSync(path.dirname(local.sourceRoot));
  const reused = prepareSource({ repoRoot: f.root, baseline: localBaseline, sourceRecord: local.recordPath });
  assert.equal(reused.sourceRoot, local.sourceRoot);
  assert.deepEqual(fs.readdirSync(path.dirname(local.sourceRoot)), preparedArchives);
  assert.equal(fs.readFileSync(path.join(localDependencies, 'dependency.txt'), 'utf8'), 'native dependency bytes');
  assert.throws(() => prepareSource({ repoRoot: f.root, baseline: { ...localBaseline, runtime: 'other' }, sourceRecord: local.recordPath }), /baseline changed/);
  assert.throws(() => prepareSource({ repoRoot: f.root, baseline: localBaseline, sourceRecord: '../foreign.json' }), /release directory/);
  const archiveLock = path.join(localDependencies, '.package-lock.json');
  fs.writeFileSync(archiveLock, '{"lockfileVersion":3}');
  assert.throws(() => prepareSource({ repoRoot: f.root, baseline: localBaseline, sourceRecord: local.recordPath }), /Dependency locks changed/);
  fs.unlinkSync(archiveLock);
  const sourceLock = path.join(f.root, 'client/node_modules/.package-lock.json');
  fs.writeFileSync(sourceLock, '{"lockfileVersion":3}');
  assert.throws(() => prepareSource({ repoRoot: f.root, baseline: localBaseline, sourceRecord: local.recordPath }), /Dependency locks changed/);
  fs.unlinkSync(sourceLock);
  const localExample = path.join(local.sourceRoot, 'client/example.txt');
  const originalExample = fs.readFileSync(localExample);
  fs.writeFileSync(localExample, 'tampered archive');
  assert.throws(() => prepareSource({ repoRoot: f.root, baseline: localBaseline, sourceRecord: local.recordPath }), /Archived source mismatch/);
  fs.writeFileSync(localExample, originalExample);
  fs.writeFileSync(path.join(f.root, 'client/example.txt'), 'dirty checkout');
  assert.throws(() => prepareSource({ repoRoot: f.root, baseline: localBaseline, sourceRecord: local.recordPath }), /Tracked source changed/);
  fs.writeFileSync(path.join(f.root, 'client/example.txt'), 'unchanged\n');
