...
export function validateRouteStop(stop) {
  const fail = (message, field) => ({ message, field });
  if (!stop.title?.trim()) return fail('כדאי להוסיף שם קצר לעצירה.', 'stopTitle');
  if (stop.editorState?.locationIncomplete || (stop.locationPrecision === 'general'
    ? !stop.destination?.countryId || !stop.destination?.cityId : !getStopCoordinates(stop))) {
    return fail('כדאי להשלים את בחירת המיקום לעצירה.', 'location');
  }
  if (normalizeRouteTimeInput(stop.editorState?.startTime ?? stop.startTime ?? '') === null) {
    return fail('השעה אינה תקינה. אפשר לכתוב למשל 09:30.', 'time');
  }
  ...
  if (rawDuration && (!Number.isSafeInteger(duration) || duration < 1 || duration > 1440)) {
    return fail('משך הביקור יכול להיות דקה ועד 24 שעות.', 'duration');
  }
  if (stop.pendingMedia?.some((item) => item.persistence === 'failed' || item.persistence === 'materializing')) {
    return fail('כדאי להשלים את טעינת התמונות או להסיר תמונה שאינה זמינה.', 'photos');
  }
  return null;
}
...
export function validateRouteComposer(draft) {
  ...
  for (const [dayIndex, day] of draft.days.entries()) {
    ...
    for (const stop of day.stops) {
      ...
      const issue = validateRouteStop(stop);
