...
export function getRecommendationDetailSections(item = {}) {
  ...
  if (budget) facts.push({ id: 'budget', icon: 'account-balance-wallet', label: budget });

  const audiences = labelsFor(facets.audiences, TRAVEL_PARTIES);
  if (facets.audienceScope === 'all') {
    facts.push({ id: 'audiences', icon: 'groups', label: 'מתאים לכולם' });
  } else if (audiences.length) {
    facts.push({ id: 'audiences', icon: 'groups', label: audiences.join(' · ') });
  }
  ...
  if (environments.length) {
    facts.push({ id: 'environments', icon: 'landscape', label: environments.join(' · ') });
  }
