...
export default function PlacesInput({ places, setPlaces }) {
    ...
    const fetchPlaces = async (query, index) => {
        ...
        try {
            ...
            const [countriesResponse, nominatimResponse] = await Promise.all([
                fetch(`https://restcountries.com/v3.1/name/${query}`).catch(() => null),
                fetch(
            ...
            if (countriesResponse) {
                ...
                if (countriesData && Array.isArray(countriesData)) {
                    const countries = countriesData.slice(0, 3).map(country => ({
            ...
            if (nominatimResponse) {
                ...
                if (citiesData && Array.isArray(citiesData)) {
                    const cities = citiesData
                        ...
                        .slice(0, 5)
                        .map(place => ({
                            name: `${place.address.city || place.address.town || place.address.village}, ${place.address.country}`,
                            type: 'city'
                        }));
    ...
    const renderItem = ({ item, index }) => {
        const idx = index + 1;
        const hasSuggestions = suggestions[index] && suggestions[index].length > 0;
        const isValid = validatedPlaces[index] === true;
        const hasInput = item && item.length > 0;
        const showError = hasInput && !isValid && !loading[index] && !hasSuggestions;
                        ...
                        {suggestions[index].map((place, i) => (
                            <TouchableOpacity
                                ...
                                style={[
                                    ...
                                    i === suggestions[index].length - 1 && { borderBottomWidth: 0 }
                                ]}
                            ...
                            >
                                <View style={styles.suggestionContent}>
                                    ...
                                    <Text style={styles.suggestionType}>
                                        {place.type === 'country' ? '🌍 Country' : '🏙️ City'}
                                    </Text>
                                </View>
                            </TouchableOpacity>
                        ))}
    ...
    const isAllValid = () => {
        return places.every((place, index) => 
